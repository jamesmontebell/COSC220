...
#include "d_stiter.h"
		...
		pair<iterator<T>, bool> insert(const T& item);
...
template <typename T>
typename stree<T>::iterator<T> stree<T>::find(const T& item)
{
	stnode<T> *curr;
...
template <typename T>
stree<T>::const_iterator<T> stree<T>::find(const T& item) const
{
	stnode<T> *curr;
...
template <typename T>
pair<typename stree<T>::iterator<T>, bool> stree<T>::insert(const T& item)
{
	...
	while(t != NULL)
	{
		...
		// bool component is false
		if (item == t->nodeValue)
			return pair<iterator<T>, bool> (iterator(t, this), false);
	...
	return pair<iterator<T>, bool> (iterator(newNode, this), true);
}

template <typename T>
void stree<T>::erase(iterator<T> pos)
{
...
template <typename T>
void stree<T>::erase(iterator<T> first, iterator<T> last)
{
	if (treeSize == 0)
 		throw
			underflowError("stree erase(): tree is empty");
...
template <typename T>
typename stree<T>::iterator<T> stree<T>::begin()
{
	stnode<T> *curr = root;
...
template <typename T>
stree<T>::const_iterator<T> stree<T>::begin() const
{
...
template <typename T>
stree<T>::iterator<T> stree<T>::end()
{
...
template <typename T>
stree<T>::const_iterator<T> stree<T>::end() const
{
	// end indicated by an iterator with NULL stnode pointer
